from .transforms import transform_balance,columns
...
def lookup_details_tron(tronObj,txinfo):
    ...
    df['block'] = res['block']
    df['txid'] = res['hash']
    df['time'] = pandas.to_datetime(res['timestamp'],unit='ms')+datetime.timedelta(hours=8)
